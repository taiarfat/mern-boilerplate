...
class AIService {
    ...
    try {
      ...
      const response = await axios.post(
        this.apiEndpoint,
        {
          model: 'gpt-3.5-turbo',
          messages: [
            { role: 'system', content: 'You are an AI assistant specialized in business analytics and executive insights.' },
  ...
  private async constructPrompt(params: InsightRequestParams): Promise<string> {
    ...
    const dateRange = `from ${startDate.toISOString().split('T')[0]} to ${endDate.toISOString().split('T')[0]}`;
