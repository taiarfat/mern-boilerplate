...
export const getRevenueChartData = async (req: AuthRequest, res: Response, next: NextFunction): Promise<Response | void> => {
  try {
    ...
    if (department) {
      // Get all projects associated with the department
      ...
      // Get income for these projects
      const departmentQuery = {
        ...query,
        project: { $in: projectIds }
      };
      
      const departmentIncomeData = await Income.aggregate([
      ...
      // Create a map of month to department total
      const departmentIncomeByMonth: Record<string, number> = {};
      
      departmentIncomeData.forEach(item => {
        departmentIncomeByMonth[item._id] = item.total;
      });
