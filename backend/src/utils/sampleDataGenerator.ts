 ...
 * @returns Promise that resolves when data is created
 */
export const generateEmployees = async (departments: any[], categories: any[], count: number = 50): Promise<any[]> => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Randomly assign a category (70% chance) or leave it undefined (30% chance)
    const category = Math.random() < 0.7 ? categories[Math.floor(Math.random() * categories.length)] : undefined;
...
export const generateAllSampleData = async (): Promise<void> => {
  try {
    ...
    // Generate employees
    const employees = await generateEmployees(departments, categories, 50);
